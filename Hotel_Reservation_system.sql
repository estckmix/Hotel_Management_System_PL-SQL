...
-- Create an index on room_id for optimization
...
CREATE TABLE bookings (
    ...
    guest_id NUMBER NOT NULL REFERENCES guests(guest_id),
    room_id NUMBER NOT NULL REFERENCES rooms(room_id),
    check_in_date DATE,
    check_out_date DATE
);

-- Step 2: Create sequences for IDs
...
CREATE OR REPLACE PROCEDURE check_in (
    ...
    v_count NUMBER;
BEGIN
    -- Ensure the booking exists and check-in date is not already set
...
BEGIN
    ...
    -- Update the check-out date if not already set
    UPDATE bookings
...
BEGIN
    ...
    SELECT COUNT(room_id) INTO v_total_rooms
    FROM rooms;
